...
impl GroupDimensions {
    ...
    /// Iterates the dimension ids in order (always at least one).
...
impl<'a> IntoIterator for &'a GroupDimensions {
    type Item = &'a String;
...
impl IntoIterator for GroupDimensions {
    type Item = String;
...
mod tests {
    ...
    #[test]
    fn group_dimensions_iteration() {
        let v = vec![String::from("FREQ"), String::from("REF_AREA")];
        let dims = GroupDimensions::new(v.clone()).unwrap();
        ...
        assert_eq!(dims.iter().map(String::as_str).collect::<Vec<_>>(), ["FREQ", "REF_AREA"]);
        assert!(IntoIterator::into_iter(&dims).eq(v.iter()));
        // Owned `IntoIterator` moves the elements out in stored order.
        assert_eq!(dims.into_iter().collect::<Vec<_>>(), v);
    }

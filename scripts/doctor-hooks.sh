...
# Check 5: Pre-push shim identity
# The installed pre-push hook must be pre-commit's generated shim, not a
# hand-rolled file. Key on pre-commit's generated-shim marker text rather than a
# byte comparison: the shim's shebang embeds a machine-specific nix-store path,
# so only the marker line is stable across environments.
echo "Checking pre-push shim identity:"

pre_push_hook=".git/hooks/pre-push"
pre_commit_marker="File generated by pre-commit"

if [ -f "$pre_push_hook" ]; then
    if grep -qF "$pre_commit_marker" "$pre_push_hook"; then
        log_ok "pre-push hook is the pre-commit-managed shim" 1
    else
        log_warn "pre-push hook is not the pre-commit-managed shim" 1
        echo "     A foreign file has replaced the shim, so pushing skips the"
        echo "     pre-push guard chain: guard-main-push (the direct-push-to-main"
        echo "     guard) and the verify-rust push-side gate."
        echo "     Run: just hook-install"
    fi
fi
# A missing pre-push hook is already reported by the hook-installation loop
# above (Check 4); this shim-identity check adds nothing for that case.

echo ""

# Check 6: Test hook execution (dry-run)
...
# Check 7: Configured hooks summary

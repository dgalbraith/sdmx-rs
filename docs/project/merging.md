...
**Branch protection rulesets enforce that all commits to `main` are GPG-signed by an authorised maintainer and that every SHA pushed to `main` carries a CI-verified green seal.** This is the cryptographic and process safeguard for the zero-trust invariant:
...
2. **Every push to `main` must carry a CI-green status check**: The Zero Trust Gate ruleset requires the single `CI Quality Gate` aggregate check to pass on the SHA being pushed. No bypass actors exist — this applies to everyone, including maintainers.

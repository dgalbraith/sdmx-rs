...
| `Ord` / `PartialOrd`             | ❌       | Deferred. `SdmxVersion` ordering is unresolved (legacy-vs-semantic equivalence), and a precedence `Ord` bound to the lossless-distinct `Eq` above would violate the `Ord`/`Eq` contract (D-0060); precedence will be an explicit method or wrapper, not a type-level `Ord`.                                |

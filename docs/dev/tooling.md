...
| `just verify-rust`           | Local & CI | Checks formatting, runs clippy and doctests, builds docs, checks semver, enforces coverage floors, and dry-runs release. | Multiple [2]                                   |
...
| `just check-conventions`     | Local      | (Supports `verify-rust`) Enforces greppable conventions in `crates/*/src` (typed `None::<T>`, empty `Vec::new()`) [3].   | `check-conventions.sh`                         |
...
[3] **Local-only member of the `verify-rust` chain.** Enforced through `verify-rust` (locally in `just verify` and on every push via the `run-just-verify-rust` pre-push hook), but with no standalone CI job. The other `verify-rust` steps each have a CI job; this greppable-convention check is deliberately left to local enforcement. See [ci-gating.md, Checks enforced locally, not in CI](../project/ci-gating.md#checks-enforced-locally-not-in-ci).
...
| `just outdated`      | Local      | Lists outdated crate versions in the workspace. Part of local `audit-all` but excluded from CI's `verify` pipeline [4]. | `cargo-outdated`                                |
| `just audit-safety`  | Local      | Scans the facade and dependency tree for `unsafe` code blocks [5].                                                      | `cargo-geiger`                                  |
...
[4] Local only non-failing check since outdated dependencies are informational, not a build failure.
[5] Local only non-failing check since `unsafe` code is expected in dependencies and reviewed separately, not a build failure.
...
| `just update-deps *CRATES`              | Maintainer | Refreshes `Cargo.lock` (semver-compatible), validates via `verify-rust`. Updates, no commit [6].                                                | `cargo update`                        |
| `just update-flake`                     | Maintainer | Refreshes `flake.lock` Nix inputs, validates via `verify-infra`. Updates, leaves unstaged, no commit [6].                                       | `nix flake update`                    |
...
[6] **Updates a lockfile but does not commit.** The bump lands in the working tree, left **unstaged**, so you can review the diff and commit a GPG-signed checkpoint manually — mirroring `changelog-generate`. Both targets also refuse to run if the lock is already dirty, so the resulting diff is solely that run's. This preserves the pinned-baseline contract: a lockfile change is a reviewed event, not an automatic side effect. See [Refreshing Pinned Dependencies](#refreshing-pinned-dependencies).
...
| `just release-dry-run *CRATES`             | Local & CI | Simulates a crate release without publishing [7].                                                                                                    | `cargo-release`                  |
| `just release-commit-changelogs`           | Maintainer | Commits generated changelogs as a signed checkpoint before cargo release.                                                                            | `git`                            |
| `just new-release-notes <version>`         | Maintainer | Scaffolds `crates/sdmx-rs/release-notes/<version>.md` from the template; refuses to overwrite a curated file [8].                                    | `scripts/new-release-notes.sh`   |
| `just check-release-notes <version>`       | Maintainer | Gate: the curated `release-notes/<version>.md` exists, has every required section, and retains no template guidance [8].                             | `scripts/check-release-notes.sh` |
...
| `just stage-merge <version>`               | Maintainer | Pushes merge commit to a `staging-release-sdmx-rs-<version>` branch, then polls the GitHub CI Quality Gate until green or failed/timed out [9].      | `scripts/release-stage.sh`, `gh` |

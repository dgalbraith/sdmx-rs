# 8. Target Version Policy for Serialization
...
Isolate outbound SDMX wire-format serialization logic from core domain models using a dedicated adapter crate (`sdmx-writers`), requiring callers to specify an explicit target version parameter to prevent cross-version compatibility bugs during output generation.

---

## Problem / Motivation

ADR-0008 establishes `ConstraintModel` as the canonical, version-agnostic domain type and commits to a bidirectional design. This requires a concrete implementation plan for the serialization direction: `ConstraintModel` → SDMX wire format.

The inbound path (wire format → `ConstraintModel`) is implemented by `sdmx-parsers`. The outbound path has no crate home and no API design. Without an explicit record, the serialization strategy risks being designed ad-hoc at implementation time, violating the adapter-pattern guardrail from ADR-0008.

Key questions that must be settled before Phase 2 implementation (the phase that implements `sdmx-parsers` and `sdmx-writers`):
* Where does serialization code live — a method on the model, the parsers crate, or a dedicated crate?
...
* **Separation of Concerns**: serialization logic must be isolated from the domain model and from parsing, per the adapter-pattern guardrail in ADR-0008.
...
* **Early Scaffold Introduction**: The crate scaffold is introduced early (Phase 0) to establish workspace structure and type contracts. Concrete serialization logic is then implemented in Phase 2 (the serialization-engine phase, alongside `sdmx-parsers`); the early scaffold lets that work proceed against locked-down contracts rather than waiting on a later phase.

---

## Proposed Design

**We will implement SDMX serialization in a dedicated `sdmx-writers` workspace crate, with the scaffold introduced early (Phase 0) to lock down type contracts and enable parallel development. Concrete serialization implementation will proceed in Phase 2 (the serialization-engine phase), alongside `sdmx-parsers`. The crate acts as a pure adapter from `ConstraintModel` — and eventually other domain types — to SDMX wire formats. Every serialization entry point accepts an explicit `TargetVersion` parameter.**
...
`SdmxSerialize` is a sealed marker trait defined in `sdmx-types`, carrying no methods initially. This bounds the API to known domain types without importing concrete serialization machinery into `sdmx-types`. If the trait acquires methods, the design must be reviewed against the `no_std + alloc` constraint in ADR-0005.
...
CSV serialization (data payloads only) is supported alongside XML and JSON. The `Csv` format is available in the `Format` enum and produces SDMX-CSV v2 output conforming to the declared `TargetVersion` (3.0 or 3.1).

The `sdmx-writers` crate is introduced as a Phase 0 scaffold in `sdmx-rs/Cargo.toml` under the `writers` feature flag (see ADR-0003), with stub trait definitions and placeholder implementations. Full serialization logic implementation occurs in Phase 2.

---

## Alternatives Considered

### Option A — serialization as methods directly on `ConstraintModel`
...
* Forces `sdmx-types` to depend on serialization libraries (e.g. `quick-xml`, `serde_json`), destroying its lightweight, `no_std + alloc` footprint.

**Verdict**: Rejected.

### Option B — serialization housed in `sdmx-parsers` (inbound crate extended to bidirectional)

Extend `sdmx-parsers` to cover both parsing and serialization.
...
* The crate name misleads contributors scanning for serialization entry points.

**Verdict**: Rejected.

### Option C — Dedicated `sdmx-writers` crate

Introduce a new `sdmx-writers` workspace crate as the serialization adapter, mirroring `sdmx-parsers` on the outbound path.
...
* **Positive**: The adapter-pattern guardrail from ADR-0008 is enforced structurally: serialization logic is physically isolated in a separate crate, making violations immediately visible at compile time.
...
* **Negative**: The `SdmxSerialize` trait boundary requires a design decision at implementation time about how much of the serialization contract is expressed in `sdmx-types` vs. `sdmx-writers`.
...
* [ADR-0009 — quick-xml and serde_json usage in sdmx-parsers; the outbound path in sdmx-writers will reuse the same libraries](../adr/0009-use-quick-xml-and-serde-json-for-streaming-deserialization.md)
* [ADR-0017 — SDMX-CSV stream parsing strategy; serialization is the symmetric outbound path](../adr/0017-sdmx-csv-stream-parsing-strategy.md)

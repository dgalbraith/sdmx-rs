...
The SDMX 3.x specification series contains structural changes between minor versions. A prominent divergence between **SDMX 3.0** and **SDMX 3.1** is the modelling of data constraints:
...
* **Separation of Concerns**: Keep wire-format version detection and payload transformation isolated within the serialisation layer (`sdmx-parsers`).
...
* **Pros**:
  ...
  * Isolates the mapping logic. The parsing engine (`sdmx-parsers`) detects the incoming schema version (e.g. via XML namespace or JSON version attributes) and normalises the payload to hydrate this enum.
  * Enables sharing common behaviours (like query matching or filter intersection) through trait implementations on `ConstraintModel`.
* **Cons**:
...
**We will model the SDMX 3.0 and 3.1 constraint divergence using a unified, version-agnostic `ConstraintModel` enum in `sdmx-types`. The `sdmx-parsers` crate will perform runtime schema detection and normalise version-specific payloads onto this model.**
...
#### 3. Version-Aware serialisation via `TargetVersion`

Every serialisation entry point must accept an explicit `TargetVersion` parameter. The caller selects the output version; the adapter handles all mapping logic internally. The conceptual API is:

```rust
let xml = model.serialize(Format::Xml, TargetVersion::V3_1)?;
```

This serialisation logic is implemented in the `sdmx-writers` crate.
...
* **Negative**: serialisation requires a `TargetVersion` decision at every call site; callers cannot omit this choice. A sensible default (e.g. latest supported version) may be provided as a convenience but must be explicit in the API contract.
...
* [Design Document 0008](../design/0008-target-version-policy-for-serialisation.md) — implementation-level record: `sdmx-writers` crate, `TargetVersion` API, and adapter design
